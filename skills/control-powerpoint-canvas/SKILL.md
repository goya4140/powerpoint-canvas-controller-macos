---
name: control-powerpoint-canvas
description: Create, recreate, revise, inspect, validate, save, or export editable Microsoft PowerPoint slides through PowerPoint's native COM canvas API. Use when the user wants live visible drawing in PowerPoint, asks for a PPT/PPTX figure or slide with editable text/shapes/arrows/pictures, rejects OS mouse or keyboard automation, requests paced step-by-step construction, or needs PNG/JPG/PDF previews rendered by PowerPoint.
---

# Control PowerPoint Canvas

Operate PowerPoint only through the `powerpoint-live` and `powerpoint-file-utils` MCP tools. Keep text, shapes, connectors, groups, and pictures as native editable objects.
...
5. Use native connectors when arrows should stay attached to moving objects. Use straight lines for exact fixed geometry.
...
7. Before delivery, call `powerpoint_file_validate`. Fix real overflow and off-slide issues; manually inspect rotated text because height metrics can be misleading.
